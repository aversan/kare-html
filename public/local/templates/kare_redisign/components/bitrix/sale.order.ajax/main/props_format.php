...
{
	function PrintPropsForm($arSource = array(), $locationTemplate = ".default", $errors = [])
	{
		if (!empty($arSource))
		{
			?>
				<div>
					...
					foreach ($arSource as $arProperties)
					{
						...
						if ($arProperties["TYPE"] == "CHECKBOX")
						{
							...
							<? if ($arProperties["CODE"] !== "SAME_AS_BUYER"): ?>
								<div class="bx_block r1x3 pt8 <?=($arProperties["CODE"] == "SUBSCRIBE" ? 'bordered-block__title bordered-block__title--mail' : '')?>">
									<?=$arProperties["NAME"]?>
							...
							<div class="bx_block r1x3 pt8 custom-checkbox">
						...
						{
							if (!empty($errors) && array_key_exists($arProperties['NAME'], $errors)) {
								ShowError($errors[$arProperties['NAME']]);
							}
							if($arProperties['CODE'] == 'USER_LOGIN' || $arProperties['CODE'] == 'CustomerPhone') {
								$phone = 'js-phone';
							}
							if ($arProperties['PARAM'] !== 'ADDRESS') :?>
								<div class="custom-input-block">
							<? endif ?>
								<input type="text" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="<?=$arProperties["NAME"]?> <?=($arProperties["REQUIED_FORMATED"]=="Y") ? '*' : ''?>" class="custom-input <?=$phone?>" <?=($arProperties['CODE'] == 'USER_LOGIN') ? 'readonly' : ''?>/>

								<?
								if (strlen(trim($arProperties["DESCRIPTION"])) > 0):
								?>
								<div class="bx_description gray-text">
						...
						elseif ($arProperties["CODE"] == "SUBSCRIBE")
						{
							?>
							<label for="<?=$arProperties["FIELD_NAME"]?>">
