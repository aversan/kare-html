...
<??>
<div class="product-card__vote iblock-vote" id="vote_<?echo $arResult["ID"]?>">
<div class="review" itemprop="aggregateRating" itemscope="itemscope" itemtype="http://schema.org/AggregateRating" style="display: none;">
...
//результатом запроса
?>
<table border="0" cellspacing="0" cellpadding="0" <?if($arResult["VOTED"] || $arParams["READ_ONLY"]):?>title="<?if($arResult["PROPERTIES"]["vote_count"]["VALUE"]) { echo GetMessage("T_IBLOCK_VOTE_RESULTS", array("#VOTES#"=>$arResult["PROPERTIES"]["vote_count"]["VALUE"] ,"#VOTE_TITLE#"=>declOfNum($arResult["PROPERTIES"]["vote_count"]["VALUE"], array("отзыв", "отзыва", "отзывов")) , "#RATING#"=>$DISPLAY_VALUE));} else {echo GetMessage("T_IBLOCK_VOTE_NO_RESULTS");}?>"<?endif;?>>
	<tr>
	<?if($arResult["VOTED"] || $arParams["READ_ONLY"]==="Y"):?>
		<?if($DISPLAY_VALUE):?>
			<?foreach($arResult["VOTE_NAMES"] as $i=>$name):?>
				<?if(round($DISPLAY_VALUE) > $i):?>
					<td><div id="vote_<?echo $arResult["ID"]?>_<?echo $i?>" class="icon-wrapper icon-wrapper_star star-voted">
						<?= \Local\Svg::get('star', 'icon icon_star') ;?>
					</div></td>
				<?else:?>
					<td><div id="vote_<?echo $arResult["ID"]?>_<?echo $i?>" class="icon-wrapper icon-wrapper_star star-empty">
						<?= \Local\Svg::get('star', 'icon icon_star') ;?>
					</div></td>
				<?endif?>
			<?endforeach?>
	...
	<?else:
		...
		<?if($DISPLAY_VALUE):?>
			<?foreach($arResult["VOTE_NAMES"] as $i=>$name):?>
				<?if(round($DISPLAY_VALUE) > $i):?>
					<td><div id="vote_<?echo $arResult["ID"]?>_<?echo $i?>" class="icon-wrapper icon-wrapper_star star-active star-voted" title="<?echo $name?>" onmouseover="voteScript.trace_vote(this, true);" onmouseout="voteScript.trace_vote(this, false)" onclick="<?echo htmlspecialcharsbx($onclick);
//11*
...
?>"></div></td>
				<?else:?>
					<td><div id="vote_<?echo $arResult["ID"]?>_<?echo $i?>" class="icon-wrapper icon-wrapper_star star-active star-empty" title="<?echo $name?>" onmouseover="voteScript.trace_vote(this, true);" onmouseout="voteScript.trace_vote(this, false)" onclick="<?echo htmlspecialcharsbx($onclick)?>">
						<?= \Local\Svg::get('star', 'icon icon_star') ;?>
					</div></td>
				<?endif?>
			<?endforeach?>
		<?else:?>
			<?foreach($arResult["VOTE_NAMES"] as $i=>$name):?>
				<td><div id="vote_<?echo $arResult["ID"]?>_<?echo $i?>" class="star-active star-empty" title="<?echo $name?>" onmouseover="voteScript.trace_vote(this, true);" onmouseout="voteScript.trace_vote(this, false)" onclick="<?echo htmlspecialcharsbx($onclick)?>">
					<?= \Local\Svg::get('star', 'icon icon_star') ;?>
				</div></td>
			<?endforeach?>
		<?endif?>
