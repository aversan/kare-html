...
for (var i = 0; i < showrooms.length; i++) {
	...
	(showrooms[i].PREVIEW_PIC ? '<div class="map-marker-info__right">' : '') +
		...
		'</p>' +
		'<a class="map-marker-info__link" href="' + showrooms[i].LINK + '">' + (showrooms[i].PARTNER == 'Y' ? 'Перейти на страницу Наших партнеров' : 'Перейти к шоу-руму') + '</a>' +
...
function setMarkers() {
	...
	for (var i = 0; i < showrooms.length; i++) {
		...
		var marker = new google.maps.Marker({
			...
			map: map,
			icon: (showrooms[i].PARTNER == 'Y' ? imageGray : image),
			title: showroom.NAME
		});
