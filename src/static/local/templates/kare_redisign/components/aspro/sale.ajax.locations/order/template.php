...
<?
	if ($arParams["LOCATION_VALUE"]) {
		$ar_location = CSaleLocation::GetByID($arParams["LOCATION_VALUE"], 'ru');
		if ($ar_location['CITY_NAME_LANG']) {
			$locationName = $ar_location['CITY_NAME_LANG'];
		}
		if ($ar_location['REGION_NAME_LANG']) {
			$locationName .= ', '.$ar_location['REGION_NAME_LANG'];
		}
		if ($ar_location['COUNTRY_NAME_LANG']) {
			$locationName .= ', '.$ar_location['COUNTRY_NAME_LANG'];
		}
	}
?>
<div class="bordered-block__form">
	<div class="custom-input-block">
		...
		<div class="bx-ui-sls-clear js_input_del"></div>
