...
<? else: ?>
	<section class="main-catalog">
		...
		<div class="main-catalog__wrapper">
			<h1 class="main-catalog__title"><?=$arSection['NAME']?></h1>
			<div class="main-catalog__left">
				<!-- дальше не редактированное -->
				...
				<? if ('Y' == $arParams['USE_FILTER']): ?>
					...
					$requestParams = array();
					if ($_REQUEST['store'])
						$requestParams['store'] = $_REQUEST['store'];
					if ($_REQUEST['order'])
						$requestParams['order'] = $_REQUEST['order'];
					if ($_REQUEST['show'])
						$requestParams['show'] = $_REQUEST['show'];
					?>
					<? $APPLICATION->IncludeComponent(
...
<!--						<div class="extended_sort"></div>-->
                        <div class="additional_sort">
                            <div class="available_filter">
                                <input class="js-action-filter" id="available-on-action" type="checkbox" name="action" value="" <?=($_REQUEST['action'] == 'Y') ? 'checked' : ''?>>
                                <label for="available-on-action" class="additional_sort_label">Сначала акционные</label>
                                <?
						...
						<div class="additional_sort">
							...
							<?
								if ($_REQUEST['store'] == 'Y') {
									...
									${$arParams["FILTER_NAME"]}['PROPERTY_227'] = 'Y';
								}
							?>
							</div>
						</div>
...
//				dump($KSHOP_SMART_FILTER, 1, 1);
				$APPLICATION->IncludeComponent(
					...
					Array(
						...
						"SECTION_ID" => $arResult["VARIABLES"]["SECTION_ID"],
						"SECTION_CODE" => $arResult["VARIABLES"]["SECTION_CODE"],
						"ELEMENT_SORT_FIELD" => !empty($_REQUEST['action'] == 'Y') ? 'CATALOG_PRICE_4' :$arParams['ELEMENT_SORT_FIELD'],
						"ELEMENT_SORT_ORDER" => $_REQUEST['order'].',nulls',
                        "ELEMENT_SORT_FIELD2" => "CATALOG_PRICE_3",
                        "ELEMENT_SORT_ORDER2" => $_REQUEST['order'].',nulls',
						"FILTER_NAME" => $arParams["FILTER_NAME"],
...
<script type="text/javascript">
	$(document).ready(function () {
        ...
        $('body').on('change', '.js-action-filter', function (e) {
            var actionFilter = 'N';
            if (this.checked) {
                actionFilter = 'Y';
            }
